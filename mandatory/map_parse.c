...
/*   Updated: 2022/05/10 15:55:02 by urycherd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	map_parce(char *argv, t_game **data)
{
	...
	fd = check_name_and_fd(argv, data);
	all_line = get_next_line(fd);
	if (all_line == NULL)
		ft_error_and_free(data, "Error: GNL mistake");
	(*data)->map_x = ft_strlen(all_line) - 1;
	while (done == 0)
		done = making_line(fd, &all_line);
	if (ft_check(all_line) == 0)
		ft_error_and_free(data, "Error: map mistake");
